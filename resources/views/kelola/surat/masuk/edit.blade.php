...
@section('content_header')
    <h1>Edit Surat Masuk ({{ Auth::user()->bidang }})</h1>
@stop
            ...
            <form action="{{url()->current()}}/post" method="post" enctype="multipart/form-data">
                <div class="card-body">
                    ...
                    @endif
                        ...
                        @endif
                    @csrf
                    <x-adminlte-input-date name="tanggal_masuk" value="{{ old('tanggal_masuk') ?? $tanggal_masuk }}"
                                           :config="$conf_tglsurat" placeholder="Masukkan Tanggal Masuk..."
                                           label="Tanggal Masuk">
                    ...
                    <x-adminlte-input name="pengirim" value="{{ old('pengirim') ?? $data->pengirim }}" label="Pengirim"
                    ...
                    @if($data->file)
                        <x-adminlte-input-file name="file" label="Upload File Surat" accept="application/pdf" placeholder="{{ old('file') ?? $data->file }}"
                                               disable-feedback/>
                    @else
